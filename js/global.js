$(document).ready(function(){
    ...
    var open = false;
    $('nav .right-nav li.notification-bar a').click(function(){
        open = !open;
        if(open){
             $(".right-nav li.notification-bar ul").fadeIn(200);
         }else{
            $(".right-nav li.notification-bar ul").fadeOut(200);
         }
    ...
    var editor_js = CodeMirror.fromTextArea(document.getElementById("code_js"), {
     ...
     $('body').on("change", '.editor', function(){
       ...
       $('nav').on("click", '.run', function(){
        ...
        var $panel = $(this).closest('.editor-area');
