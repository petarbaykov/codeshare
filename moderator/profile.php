      ...
      <?php 
      	$stm = $DB_con->prepare("SELECT follower FROM follow WHERE followe=:userid");
					...
					while($followers = $stm->fetch(PDO::FETCH_ASSOC)){
						...
						if($userid != $user_id){
						if($num_rows == 1){
							echo "<a href='follow.php?follow=false&user_id=$userid' class='followship unfollow'>Следван</a>";
						}else{
							echo "<a href='follow.php?follow=true&user_id=$userid' class='followship follow'>Следвй</a>";
						}
					}
							echo "<a href='user_profile.php?user_id=$userid'><img src='images/$image'></a>";
							echo "<a href='user_profile.php?user_id=$userid'><p>$fname $lname</p></a>";
							
							echo "<a href='user_profile.php?user_id=$userid'><p>@$user_name</p></a>";
							echo "</section>";
						}
		}
      	...
      	$stm = $DB_con->prepare("SELECT followe FROM follow WHERE follower=:userid");
					...
					while($following = $stm->fetch(PDO::FETCH_ASSOC)){
						...
						if($num_rows == 1){
							...
							echo "<a href='follow.php?follow=true&user_id=$userid' class='followship follow'>Следвай</a>";
						}
					}
